...
const isValueSet = (value: string | boolean | null | undefined): boolean => {
  ...
  if (typeof value === 'string') return value.trim() !== '';  return true;
};
...
export default function WordDetail() {
  ...
  useEffect(() => {
    ...
    if (stored) {
      setIsFavorite(new Set(JSON.parse(stored)).has(id!));
    }
  }, [id]);

  const toggleFavorite = useCallback(() => {    const stored = localStorage.getItem('nzebi_favorites');
