...
export default function Index({ searchTerm }: IndexProps) {
  ...
  // Sort words based on display mode
  const sortedWords = useCallback((): Word[] => {
    const wordsCopy = [...words];
    if (displayMode === 'french-first') {
      return wordsCopy.sort((a, b) => a.french_word.localeCompare(b.french_word, 'fr'));
    } else {
      return wordsCopy.sort((a, b) => a.nzebi_word.localeCompare(b.nzebi_word, 'fr'));
    }
  }, [words, displayMode]);

  const sortedWordsList = sortedWords();
