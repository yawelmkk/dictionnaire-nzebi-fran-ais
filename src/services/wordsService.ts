...
const loadAllWordsInternal = async (): Promise<Word[]> => {
  ...
  if (wordsFromJson.length > 0) {
    ...
    cacheWordsToIndexedDB(wordsFromJson).catch(err => console.error('IndexedDB cache error:', err));    console.log(`Dictionnaire chargé: ${wordsFromJson.length} mots depuis dictionnaire.json`);
