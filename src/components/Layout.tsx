  ...
  const handleSettingsClick = () => {
    toast(
      <div className="flex flex-col gap-4 max-w-md">
        <div>
          <h3 className="font-bold text-lg mb-4">Paramètres d'affichage</h3>
          <div className="space-y-3">
            <div>
              <label className="block text-sm font-medium mb-2">Mode d'affichage des mots</label>
              <select
                value={displayMode}
                onChange={(e) => {
                  setDisplayMode(e.target.value as 'nzebi-first' | 'french-first');
                  toast.success(e.target.value === 'nzebi-first' 
                    ? 'Affichage : Nzébi en premier' 
                    : 'Affichage : Français en premier');
                }}
                className="w-full border rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-800 dark:border-gray-600 dark:text-white"
              >
                <option value="nzebi-first">Nzébi d'abord (défaut)</option>
                <option value="french-first">Français d'abord (clic pour nzébi)</option>
              </select>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                {displayMode === 'nzebi-first' 
                  ? 'Les mots nzébi s\'affichent avec traductions françaises.'
                  : 'Les mots français s\'affichent, les définitions nzébi apparaissent au clic.'}
              </p>
            </div>
          </div>
        </div>
    ...
    // Keep the secret feature to access word management
